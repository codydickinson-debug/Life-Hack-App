...
- **Privacy-mode (`👁` eye icon in the topbar) blurs every dollar amount.** It works by targeting `[data-priv="amount"]` in CSS. **Every user-visible `${fmt$0(...)}` in HTML must be wrapped in a `<span data-priv="amount">…</span>`** (or use `data-priv="amount"` on the parent if it contains nothing else). Skip only for non-rendered contexts: toast strings, Cylan seed prompts, AI message bodies. Coverage audit script: `grep -c 'data-priv="amount"' index.html` should track close to the count of user-visible `fmt$0(` callsites.
